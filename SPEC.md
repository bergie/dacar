...
For forward-secret, point-to-point delivery to offline nodes (bypassing the public RFed broadcast):
 ...
 2. **The Wrapper:** The Delta Push payload is embedded in an LXMF message with the exact title `dacar/sync/delta`. A node MAY alternatively send a **batch**: an LXMF message titled `dacar/sync/batch` whose content is a MessagePack **array** of Delta Push payloads (§5.3), packed to the transport's size limit. The batch envelope is pure packing and adds no trust or ordering: a receiving node MUST decode the array and pass **every** element through the same verify-on-ingest as a lone Delta (§11.2.4), applying valid elements and dropping invalid ones individually — never all-or-nothing. Nodes MUST accept both titles.
